...
class EExcelWriter extends CGridView {
    ...
    private $rowFormat;
    private $columnLengths = array();
    private $snookPath = "/var/www/intradox3/assets/";
    ...
    public function init() {
        ...
        $this->initColumns();
        $this->workBook = new writeexcel_workbookbig($this->snookPath . $this->fileName);
    ...
    public function renderHeader() {
        ...
        foreach ($this->columns as $column) {
            ...
            $this->columnLengths[$this->currentCol] = strlen($head);
            $this->currentCol++;
        }
        $this->currentRow++;
    }
    ...
    public function renderRow($row) {
        ...
        foreach ($this->columns as $n => $column) {
            ...
            if ($this->columnLengths[$this->currentCol] < strlen($value)) {
                $this->columnLengths[$this->currentCol] = strlen($value);
            }
            $this->currentCol++;
        }
        $this->currentRow++;
    ...
    public function run() {
        ...
        if ($this->stream) { //output to browser
            ...
            $fh = fopen($this->snookPath . $this->fileName, "rb");
            fpassthru($fh);
        }
    }
