...
class writeexcel_worksheet extends writeexcel_biffwriter {
    ...
    function writeexcel_worksheet($name, $index, &$activesheet, &$firstsheet, &$url_format, &$parser, $tempdir) {
...
#
    ...
    function set_header($string, $margin) {
        ...
        if (strlen($string) >= 255) {
            trigger_error("Header string must be less than 255 characters", E_USER_WARNING);
            return;
        }
    ...
    function set_footer($string, $margin) {
        if (strlen($string) >= 255) {
            trigger_error("Footer string must be less than 255 characters", E_USER_WARNING);
            return;
        }
...
#
    ...
    function write() {
        ...
        /**
         * Commented out  as it was breaking with urls in the notes field, possibly
         * due to the new line at the or perhaps it expects a a.text attribute.
         * Don't need, so don't care
         *
         * @date 2018-04-18
         * @author John Snook
         */
...
#
    ...
    function _position_image($col_start, $row_start, $x1, $y1, $width, $height) {
        ...
        if ($this->_size_col($col_start) == 0) {
            return;
        }
        if ($this->_size_col($col_end) == 0) {
            return;
        }
        if ($this->_size_row($row_start) == 0) {
            return;
        }
        if ($this->_size_row($row_end) == 0) {
            return;
        }
        ...
        $this->_store_obj_picture($col_start, $x1, $row_start, $y1, $col_end, $x2, $row_end, $y2);
    }
    ...
    function _store_obj_picture($col_start, $x1, $row_start, $y1, $col_end, $x2, $row_end, $y2) {
    ...
    function _process_bitmap($bitmap) {
        ...
        if (strlen($data) <= 0x36) {
            trigger_error("$bitmap doesn't contain enough data.", E_USER_ERROR);
        }
        ...
        if ($width > 0xFFFF) {
            trigger_error("$bitmap: largest image width supported is 64k.", E_USER_ERROR);
        }

        if ($height > 0xFFFF) {
            trigger_error("$bitmap: largest image height supported is 64k.", E_USER_ERROR);
        }
        ...
        if ($bitcount != 24) {
            trigger_error("$bitmap isn't a 24bit true color bitmap.", E_USER_ERROR);
        }

        if ($planes != 1) {
            trigger_error("$bitmap: only 1 plane supported in bitmap image.", E_USER_ERROR);
        }
        ...
        if ($compression != 0) {
            trigger_error("$bitmap: compression not supported in bitmap image.", E_USER_ERROR);
        }
