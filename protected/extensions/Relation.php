...
class Relation extends CWidget {
	...
	public function renderManyManyDropDownListSelection() {
		...
		foreach ($this->_relatedModel->findAll() as $obj) {
			...
			echo CHtml::dropDownList('rel-' . $obj->id . "-" . $this->getListBoxName(), $isAssigned ? $obj->id : 0, CHtml::listData(
							array_merge(
									array('0' => $this->allowEmpty), $this->_relatedModel->findAll()), $this->relatedPk, $this->fields
					)
			);
